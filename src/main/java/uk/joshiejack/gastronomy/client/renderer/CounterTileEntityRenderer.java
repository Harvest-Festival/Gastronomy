...
@OnlyIn(Dist.CLIENT)
public class CounterTileEntityRenderer extends AbstractCookwareTileEntityRenderer<CounterTileEntity> {
    public CounterTileEntityRenderer(TileEntityRendererDispatcher dispatcher) {
        super(dispatcher);
    }

    @Override
    public void render(@Nonnull CounterTileEntity tile, float partialTicks, @Nonnull MatrixStack matrix, @Nonnull IRenderTypeBuffer buffer, int combinedLightIn, int combinedOverlayIn) {
        super.render(tile, partialTicks, matrix, buffer, combinedLightIn, combinedOverlayIn);
    }

    @Override
    protected void applyResultTransformation(@Nonnull MatrixStack mtx, @Nonnull CounterTileEntity tile) {
        mtx.translate(0.5F, 0.5F, -1.05F);
        mtx.scale(0.5F, 0.5F, 0.5F);
        mtx.mulPose(Vector3f.ZP.rotationDegrees(180 + tile.getBlockState().getValue(HorizontalBlock.FACING).toYRot()));
    }

    @Override
    protected void applyIngredientTransformations(@Nonnull MatrixStack mtx, @Nonnull CounterTileEntity tile, int i) {
        mtx.translate(0.5F, 0.5F, -1.05F);
